...
class MinesweeperAI():
                ...
                if 0 <= i < self.height and 0 <= j < self.width:
                    if (i, j) not in self.safes:
                        if (i, j) in self.mines:
                            ...
                            neighbrs.add((i, j))
        self.knowledge.append(Sentence(neighbrs, count))
        ...
        while upd:
            ...
            for sentence in self.knowledge:
                if sentence.known_safes():
                    x = sentence.known_safes().copy()
                    for cell in x:
                        self.mark_safe(cell)
                    upd = True
                if sentence.known_mines():
                    x = sentence.known_mines().copy()
                    for cell in x:
                        self.mark_mine(cell)
                    upd = True
                        ...
                        if s1 != s2:
                            if  s1.cells.issubset(s2.cells):
                                ...
                                for s in self.knowledge:
                                    if s.cells == newcells and s.count == newcount:
                                        already = True
                                if not already:
    ...
    def make_safe_move(self):
        ...
        """
        if self.safes and self.moves_made:
            for elem in self.safes:
                if elem not in self.moves_made:
                    return elem
        return None
